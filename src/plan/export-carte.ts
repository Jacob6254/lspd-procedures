...
import { chargerImage, couperTexte, fleche, icone, pastille, plaque, sortie, type Sortie } from './dessin'
...
export async function exporterCarte(op: Operation): Promise<Sortie> {
  ...
  const dessins = new Map<string, HTMLImageElement | null>()
  await Promise.all(
    vus.map(async (m) => {
      const c = teinte(op, m.uniteId)
      const cle = `${m.type}|${c}`
      if (dessins.has(cle)) return
      dessins.set(cle, await icone(defMarqueur(m.type).icone, c))
    })
  )
  for (const m of vus) {
    const c = teinte(op, m.uniteId)
    pastille(ctx, px(m.x), py(m.y), r, c, defMarqueur(m.type).code, dessins.get(`${m.type}|${c}`))
  }
  ...
  const hNom = 26 * ech
  // Les pastilles comptent aussi comme obstacles : un nom ne doit pas en couvrir une.
  const posees: { x: number; y: number; larg: number; haut: number }[] = vus.map((m) => ({
    x: px(m.x),
    y: py(m.y),
    larg: r * 2.1,
    haut: r * 2.1
